...
以下数据建议基于本地或测试环境压测填写，主要用于展示优化方向和实际收益，不代表生产环境极限容量。
...
- Redis + Lua 在入口完成库存预扣和一人一单拦截
- 分片 Set 方案下，重复请求不会继续打到数据库
- 消费端幂等保证异步落库阶段不会生成重复订单
...
- 通过订单明细批量查询 + 内存分组装配，消除 N+1 问题
- 订单数据量上来后，响应时间和 SQL 次数更稳定

### 3. 热点订单缓存效果

| 指标 | 未命中缓存 | 命中本地/Redis 缓存 |
|---|---:|---:|
| 平均响应时间 | `待补充` | `待补充` |
| P95 响应时间 | `待补充` | `待补充` |
| 缓存命中率 | - | `待补充` |
...
- Caffeine + Redis 二级缓存显著减少热点数据回源数据库的次数
- 配合失效广播，多实例下缓存一致性可控

### 4. 压测口径建议

- 秒杀链路：固定活动 ID 和商品 ID，分别测试“不同用户抢购”和“同用户重复请求”两组流量
- 订单分页：分别准备 10、100、1000 条订单数据，对比优化前后的 SQL 次数和响应时间
- 缓存效果：分冷启动和热缓存两轮测试，记录平均耗时、P95 和命中率
- 所有结果建议统一记录总请求数、成功数、失败数、平均耗时、P95、峰值吞吐
